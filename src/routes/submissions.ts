  ...
  try {
    ...
    if (req.files) {
      Object.entries(req.files as fileUpload.FileArray).forEach((file) => {
        ...
        (file[1] as any).mv(__dirname, process.env.RAILWAY_VOLUME_MOUNT_PATH ?? "/uploads/" + name);
      });
    }
    const submitee =
...
router.get("/imgs/:name", async (req, res) => {
  ...
  return res.sendFile(path.join(process.env.RAILWAY_VOLUME_MOUNT_PATH ?? "/uploads/", name));
});
